...
class EditController extends AbstractController
{
    ...
    public function updateAction(User $user)
    {
        $this->redirectIfDirtyObject($user);
			      ...
			      //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($user);exit();die; 
			      //justin updated - to be added to affiliates extension one day
			      //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($user);exit();die; 
			      
			      
			      $images = $user->getImage();
			      $imageArray = explode(",", $images);
			      $user->setImage(array_shift(array_values($imageArray)));
