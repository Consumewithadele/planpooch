...
class lastRSS {
	...
	var $xmlproptags = array( 'enclosure' => array( 'url', 'length', 'type' ) );
	...
	function Parse ($rss_url) {
		// Open and load RSS file
            ...
            $rss_httpmsg = \TYPO3\CMS\Core\Utility\GeneralUtility::getURL($rss_url, 1, FALSE, $report);
            // Check for "301 Moved"
            while ($report['http_code'] == '301') {
                if ( $rss_httpmsg ) {
                    ...
                    if (isset($location[1])) {
                        $rss_httpmsg = \TYPO3\CMS\Core\Utility\GeneralUtility::getUR($location[1], 1, FALSE, $report);
                    }
                }
            }
            if ( $rss_httpmsg ) {
					...
					foreach($this->xmlproptags as $xmlpropkey => $xmlproperties ) {
						...
						if ($temp != '') {
                                                  foreach($xmlproperties as $xmlproperty) {
                                                          ...
                                                          if ($tempprop != '') $result['items'][$i][$xmlpropkey]['prop'][$xmlproperty] = $tempprop; // Set only if not empty
                                                  }
                                                }
					}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/gkh_rss_import/lib/lastRSS.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/gkh_rss_import/lib/lastRSS.php']);
}
